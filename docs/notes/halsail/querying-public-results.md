...
login-free, but it is *not* the same app — different URL scheme, an older
...
a series `33`) — they only mean anything inside their dataset (`{ds}`).
...
| `GET /Result/_CrsSeryDropDown/{ds}?ClassKey={c}` | The **series available to that class** — the class→series join. Option `value` = SeriesKey. |
| `GET /Result/_CrsResults/{ds}?SeriesKey={s}` | The **results table** fragment. SeriesKey is unique per `(class × series)` pair, so it already encodes the class. |

The entry page is `GET /Result/Public/{ds}` (or `/{ds}/{seriesKey}`) — a shell
...
### Why this matters: the class→series join is invisible to results pages

On the live site, one fetch of any `/Result/Public/{id}` shell hands you the
whole `fleet → [series]` catalog statically (the `#dd{fleetId}` selects). **The
...
every class and persist it — it is the join that reveals how the season is
...
1. **Map** (`_CrsResultSetDropDown` → per-dataset `_CrsClassDropDown` → per-class
   `_CrsSeryDropDown`): builds the full class→series join cheaply (~150 requests
...
- `GET /Result/_CrsResultSetDropDown/1426?DSKey=3413` → datasets `3413` (2025),
  `3264` (2024), `3206` (2023), `3123` (2022).
- `GET /Result/_CrsClassDropDown/3413` → 56 classes incl. `14` *"Cruisers 0 IRC"*.
- `GET /Result/_CrsSeryDropDown/3413?ClassKey=14` → that class's series:
  *Thursday Overall, Saturday Overall, Thursday/Saturday Series A & B, 2025 RAYC
  Super League 1 & 2* — i.e. one fixed-handicap class spanning both days. By
  contrast `ClassKey=13` *"Cruisers 0 Echo (Thu)"* lists only the three Thursday
  series (its ECHO chain is per-day).
- `GET /Result/_CrsResults/3413?SeriesKey=33` → the *Cruisers 0 Echo (Thu),
