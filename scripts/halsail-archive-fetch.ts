 ...
 * `sources/`. The Hal *archive* is a separate app from the live HalSail site,
 * with a four-level AJAX cascade (all public GET, no auth/token):
 *
...
const OUT_DIR = join(__dirname, '..', 'sources');
const DELAY_MS = 750;
const HEADERS = {
  'User-Agent': 'Mozilla/5.0 (dbsc-archive capture; +parity)',
...
/** Stage B: fetch the results table for every distinct (class×series) pair.
 *  SeriesKey is unique per pair, so dedupe by key to avoid redundant pulls. */
async function fetchResults(catalog: Catalog): Promise<void> {
  const yearDir = join(OUT_DIR, catalog.year);
  const seen = new Set<string>();
  const pairs = catalog.classes.flatMap((c) => c.series);
  console.log(`\n=== ${catalog.name}: ${pairs.length} pairs → results ===`);
  for (const s of pairs) {
    if (seen.has(s.key)) continue;
    seen.add(s.key);
    await fetchCached(
      `${BASE}/Result/_CrsResults/${catalog.dsKey}?SeriesKey=${s.key}`,
      join(yearDir, `results/series-${s.key}.html`),
    );
  }
}
...
async function main() {
  ...
  console.log(`Hal archive capture${mapOnly ? ' (map-only)' : ''} → ${OUT_DIR}`);
  ...
  if (!mapOnly) for (const c of catalogs) await fetchResults(c);
  ...
  console.log(`\n— summary (${requestCount} requests made) —`);
  for (const c of catalogs) {
    console.log(
      `  ${c.year}: ${c.counts.classes} classes, ${c.counts.pairs} (class×series) pairs`,
    );
  }
}
